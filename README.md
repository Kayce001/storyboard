...
这个项目用于把一份中文讲解文案，转换成可审阅的分镜提示词，并进一步生成带旁白、字幕、片头片尾和 BGM 的成片。
...
项目当前的稳定默认路径仍然偏向手工模式，也就是“你提供图片，我来稳定出片”。自动生图链路已经保留，并有单独入口。

## 这是什么

项目当前主要解决三件事：

1. 把原始 `txt` 文案整理成可审阅的 `prompt_pack.md / prompt_pack.json`
2. 在已有分镜图的前提下，自动生成旁白、字幕和最终视频
3. 在需要时，提供端到端自动生图再出片的完整流水线

## 推荐环境

当前推荐运行环境是 **WSL + `.venv-linux`**。
...
- 这个脚本只接受原始 `txt`
- 它不会从 `output/runs/.../segments_*.json` 反推 prompt pack
- 如果你改了 `tasks/4.txt`，想刷新提示词，就重新跑这条
...
```bash
python scripts/run_full_pipeline.py --input-file tasks/<任务名>.txt --config config/providers.json
```
...
- `scripts/`：命令入口脚本
- `config/`：运行时配置
- `prompts/`：运行时 prompt 模板与后缀模板
- `assets/`：片头片尾、BGM 等可复用资源
- `tasks/`：真实任务输入区

### 协作辅助目录

- `skills/`：给 Codex 或人工协作者看的工作流说明与参考资料
...
- 系统不额外补一张多余的总结图
- 但如果原文本身有总结段，会尽量保留到最后一张

### Prompt pack 必须从原始 txt 重建
...
- 用 `output/runs/.../segments_*.json` 倒推当前 prompt pack
- 用旧 run 覆盖当前 `workbench`
