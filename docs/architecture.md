...
当前新增的 plus 链路是“文本 -> 简洁分镜骨架 -> 优化生图提示词与口播 -> 手动放图 -> 讲解视频”。它是增强路径，不替代原始主路径。
...
8. plus 链路只增强提示词和口播层，不改变原有 `tasks/`、`output/workbench/`、`output/runs/` 主路径语义。
...
1. 把文案放到 `tasks_plus/<name>/<name>.txt`。
2. 运行 `scripts/run_prompt_pack_plus_wsl.sh <name> --task-name <name>`。
3. 审阅 `output/workbench_plus/<name>/image_prompt_pack.md`。
4. 手动生成图片，放回 `tasks_plus/<name>/`，命名为 `1.png`、`2.png`、`3.png`。
5. 运行 `scripts/run_make_video_plus_wsl.sh <name>` 或 `scripts/run_make_video_plus_body_only_wsl.sh <name>`。
...
- `tasks_plus/<name>/`
  plus 链路的任务文本和图片目录。文本通常是 `<name>.txt`，图片按 `1.png`、`2.png`、`3.png` 放置。
...
- `output/workbench/<name>/prompt_pack.json`
  机器可读的分镜规划数据。
- `output/workbench_plus/<name>/image_prompt_pack.md`
  plus 链路的人可读生图提示词包，结构是“图几 -> 原始内容片段 -> 生图提示词”。
- `output/workbench_plus/<name>/narration_script.txt`
  plus 链路的人可读口播稿，结构是“图几 -> 原始内容片段 -> 优化口播”。
- `output/workbench_plus/<name>/video_strategy.json`
  plus 链路的视频策略数据，供渲染阶段读取。
...
- `output/runs_plus/<name>/`
  plus 链路的历史运行记录，结构与 `output/runs/<name>/` 类似。
...
- `scripts/rebuild_prompt_pack_plus.py`
  从原始 `txt` 和基础分镜骨架生成 plus 资产，包括优化后的生图提示词、口播稿和视频策略。
- `scripts/make_video_plus.py`
  复用 `scripts/make_video.py` 的渲染能力，但读取 `output/workbench_plus/<name>/` 和 `tasks_plus/<name>/`。
- `scripts/run_prompt_pack_plus_wsl.sh`
  Ubuntu 包装脚本，用任务名快速生成 plus 提示词包。
- `scripts/run_make_video_plus_wsl.sh`
  Ubuntu 包装脚本，用任务名快速生成 plus 视频。
- `scripts/run_make_video_plus_body_only_wsl.sh`
  Ubuntu 包装脚本，生成不拼接片头片尾的 plus 正文视频。
...
- 默认模型：由 `config/providers.json` 控制。文档不记录私有转发链路细节，只要求配置提供 OpenAI-compatible 文本生成能力。
...
当前优先使用 Ubuntu 作为 TTS 运行环境：

- Windows 侧启动时可以先走 `edge_tts_wsl`，再 fallback 到 `edge_tts`
- Ubuntu 环境内运行时会自动跳过 `edge_tts_wsl`，直接使用本地 `edge_tts`
...
- 文件：`src/storyboard_video/pipeline/prompt_pack_plus.py`
- 输出：`output/workbench_plus/<name>/image_prompt_pack.md`、`narration_script.txt`、`video_strategy.json`、`prompt_pack_plus.json`

plus 链路复用基础分镜骨架，但不照搬原始 prompt pack 的复杂输出。它面向人工生图和后续合成，只保留最关键的信息：
...
当前 plus 生图 prompt 的结构：

- 先从基础分镜生成 `visual_goal`、`attention_hook`、`composition_type` 三个轻量视觉字段。
- 再让模型根据这三个字段优化最终生图提示词。
- 最后统一追加很短的 16:9、安全区、中文准确性约束。

这层的目标是让所有任务的图片更有停留感，但保持链路简洁，不增加质量评分器或二次重写器。

首帧 hook 保护：

- 如果首段包含 URL，会先清理链接，避免把 `https`、`www` 或仓库地址残片当成标题。
- 如果识别到 GitHub 仓库名，会优先生成“主题 + 到底是什么”一类中文问题。
- 如果抽出的标题太短、纯符号或明显不可读，会回退到更安全的问题句。

这不是任务特判，而是首帧标题的防呆层。正常中文首句仍优先沿用原始问题。
...
Ubuntu TTS 已经接入为正式后端：

- Windows 侧启动时默认先走 `edge_tts_wsl`
- Ubuntu 环境内运行时直接走本地 `edge_tts`
